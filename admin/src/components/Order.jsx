...
import { statusKeyMap } from "../assets/dummyadmin";
...
const Order = () => {
  ...
  const handleStatusChange = async (orderId, newStatusKey) => {
    try {
      ...
      setOrders(
        orders.map((o) =>
          o._id === orderId ? { ...o, status: newStatusKey } : o
        )
      );
    } catch (err) {
  ...
  if (error) return <div className="text-red-400 text-xl ">{error}</div>;
                ...
                {orders.map((order) => {
                  ...
                  return (
                    <tr key={order._id} className={tableClasses.row}>
                      ...
                      <td className={tableClasses.cellBase}>
                        <div className="space-y-1 max-h-52 overflow-x-hidden overflow-y-hidden">
